...
router.post("/signup", (req, res) => {
  ...
  if (global.MOCK_DB) {
    return res.status(201).json({ result: "User signed up successfully (Mock)" });
  }
...
router.post("/login", (req, res) => {
  ...
  if (global.MOCK_DB) {
    const mockUser = {
      ...
      street: "Mock Street",
      homeAddress: "Mock City",
      state: "Mock State",
      country: "Mock Country",
      aboutMe: "I am a mock user running without database.",
      followers: [],
      following: [],
      role: "student",
    };
    const jwtToken = jwt.sign({ _id: mockUser._id }, JWT_SECRET);
    return res.status(200).json({ result: { token: jwtToken, user: mockUser } });
  }
        ...
        .then((didMatch) => {
          if (didMatch) {
            ...
            const userInfo = {
              ...
              role: userInDB.role,
              activityLog: userInDB.activityLog,
            };
...
router.put("/updatedata/:_id", async (req, res) => {
  try {
    ...
    if (updatedFields.password) {
      const hashed = await bcryptjs.hash(updatedFields.password, 10);
...
router.get("/users", protectedRoute, adminMiddleware, async (req, res) => {
  try {
    const allusers = await UserModel.find().select("-password"); // Exclude passwords!
...
// REST API to log daily activity for the heatmap
router.post("/activity", protectedRoute, async (req, res) => {
  try {
    const today = new Date();
    // Setting to midnight to count as 1 block per day conceptually, although we just store full dates.
    // The calendar heatmap just looks at unique date strings.
    const loggedInUser = await UserModel.findByIdAndUpdate(
      req.user._id,
      { $push: { activityLog: today } },
      { new: true }
    ).select("-password");

    res.json(loggedInUser.activityLog);
  } catch (error) {
    console.error(error);
